        ...
        <div className="container-main py-20 flex flex-col md:flex-row items-center gap-12 relative z-10">
          <div className="flex-1 text-left">
            ...
            >
              ...
              <h2 className="text-3xl md:text-4xl text-secondary font-heading mb-8">
                Welcome to my corner of the internet.
              </h2>
              <p className="text-xl md:text-2xl text-text mb-8 leading-relaxed font-light">
                I’m a tech marketer turned Yale SOM MBA Candidate with a passion for building.
              </p>
              <p className="text-body text-text/80 mb-10 leading-relaxed max-w-2xl text-lg">
                I’ve been coding since college, but lately, I’ve been focused on <span className="font-bold text-accent">vibe coding</span>—using modern tools to bring ideas to life at the speed of thought. This site is my playground and portfolio for everything I’m building during my time at Yale.
              </p>
              
              {/* Organizations */}
              <div className="mt-16">
                <h3 className="text-sm uppercase tracking-widest font-bold text-secondary mb-6">Organizations I've been part of</h3>
                <div className="flex flex-wrap gap-8 items-center">
                  {orgLogos.map((logo, idx) => (
                    <img 
                      ...
                      src={logo.src} 
                      alt={logo.alt} 
                      className="h-8 md:h-12 w-auto opacity-60 hover:opacity-100 grayscale hover:grayscale-0 transition-all duration-300 hover:scale-110"
                    />
                  ))}
                </div>
              </div>
          ...
          <div className="flex-1 w-full h-[500px] md:h-[600px]">
            <LogoBubbles />
          </div>
        </div>
      </section>
      ...
      <section id="experience" className="py-24 bg-secondary/5 relative overflow-hidden">
        <div className="container-main relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20">
            ...
            >
              <h2 className="text-4xl font-bold mb-12 text-text border-b-4 border-accent inline-block pb-2">Professional Experience</h2>
              <div className="space-y-12 relative border-l-4 border-secondary/30 pl-10 ml-4">
                {experience.map((exp, idx) => (
                  <div key={idx} className="relative group">
                    ...
                    <p className="text-xl text-secondary font-medium mb-3">{exp.role}</p>
                    {exp.description && <p className="text-text/80 text-lg leading-relaxed">{exp.description}</p>}
                  </div>
                ))}
              </div>

              <h2 className="text-4xl font-bold mb-12 mt-20 text-text border-b-4 border-accent inline-block pb-2">Education</h2>
              <div className="space-y-12 relative border-l-4 border-secondary/30 pl-10 ml-4">
                {education.map((edu, idx) => (
                  <div key={idx} className="relative group">
                    <div className="absolute -left-[54px] top-1 w-7 h-7 rounded-full bg-accent border-4 border-background transition-transform group-hover:scale-125 duration-300"></div>
                    <h3 className="text-2xl font-bold text-primary mb-2">{edu.school}</h3>
                    <p className="text-xl text-text/80">{edu.degree}</p>
                  </div>
                ))}
              </div>
            </motion.div>
        ...
        <div className="container-main text-center relative z-10">
          ...
          >
            ...
            <div className="flex justify-center gap-8">
              <a href="https://www.linkedin.com/in/candiceshen/" target="_blank" rel="noopener noreferrer" className="p-5 bg-white rounded-full shadow-xl hover:scale-110 transition-transform text-[#0077b5] group">
                <svg className="w-10 h-10 group-hover:rotate-12 transition-transform duration-300" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/></svg>
              </a>
              <a href="https://github.com/candicesxc" target="_blank" rel="noopener noreferrer" className="p-5 bg-white rounded-full shadow-xl hover:scale-110 transition-transform text-black group">
                <svg className="w-10 h-10 group-hover:-rotate-12 transition-transform duration-300" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path fillRule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" clipRule="evenodd" /></svg>
              </a>
            </div>
